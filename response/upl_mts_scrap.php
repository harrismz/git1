	...
	*/
	...
	if ($validfile == false) {
		...
		try
		{
			...
			$rs = $db->Execute("delete from tbl_mts_scrp where scrap_periode = '{$periode}'");
			$rs->Close();			
			...
			if (is_uploaded_file($_FILES['ufile']['tmp_name'])) 
			{
				...
				while ($i = fgetcsv($feed, 1000, ",")) 
				{
					if(trim($i['0']) != "Kode Barang"){
						...
						$rs = $db->Execute("insert 	into tbl_mts_scrp(kodebarang, namabarang, jumlah, scrap_periode, action_user)
