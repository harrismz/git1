...
require_once('../extjs/con_iis.php');
	...
	if ($validfile == false) {
		...
		try
		{
			...
			$rs = $db->Execute("delete from tbl_mts_svcpart where mtr_periode = '{$periode}'");
			$rs->Close();			
			...
			if (is_uploaded_file($_FILES['ufile']['tmp_name'])) 
			{
				...
				while ($i = fgetcsv($feed, 1000, ",")) 
				{
					if(trim($i['0']) != "Kode Barang"){
						...
						$rs = $db->Execute("insert 	into tbl_mts_svcpart(kodebarang, namabarang, sal_awal, masuk, keluar, sal_akhir, mtr_periode, action_user)
