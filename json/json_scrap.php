...
require_once('../extjs/con_iis.php');
	...
	if($val_periode != ""){
		$rs = $db->execute("declare @totalcount as int
							exec searchscrap $start, $limit, '". $val_periode ."', @totalcount=@totalcount out");
		...
		$rs = $db->execute("declare @totalcount as int
							exec displayscrap $start, $limit, @totalcount=@totalcount out");
		$totalcount = $rs->fields['13'];
		$return = array();
	}
