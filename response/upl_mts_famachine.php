	...
	*/
	...
	if ($validfile == false) {
		...
		try
		{
			...
			$rs = $db->Execute("delete from tbl_mts_famach where famach_periode = '{$periode}'");
			$rs->Close();			
			...
			if (is_uploaded_file($_FILES['ufile']['tmp_name'])) 
			{
				...
				while ($i = fgetcsv($feed, 1000, ",")) 
				{
					if(trim($i['0']) != "Kode Barang"){
						...
						$rs = $db->Execute("insert 	into tbl_mts_famach(kodebarang, namabarang, sal_awal, masuk, keluar, sal_akhir, famach_periode, action_user)
											select	'{$kodebarang}', '{$namabarang}', '{$sal_awal}', '{$masuk}', '{$keluar}', '{$sal_akhir}', '{$periode}', '{$action_user}'");
						$rs->Close();
					}
