	...
	*/
	...
	if ($validfile == false) {
		...
		try
		{
			...
			$rs = $db->Execute("delete from material where periode = '{$periode}'");
			$rs->Close();			
			...
			if (is_uploaded_file($_FILES['ufile']['tmp_name'])) 
			{
				...
				while ($i = fgetcsv($feed, 1000, ",")) 
				{
					if(trim($i['0']) != "No"){
						...
						$rs = $db->Execute("insert 	into material(no, kodebarang, namabarang, unit, last, rcv, adj, iss, bal, stokopname, diff, periode)
