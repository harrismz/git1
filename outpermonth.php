...
/*
	...
	****	on 31 May 2017
	****	revise: menambahkan upload ubah no aju ke no pendaftaran BC25 (10 February 2017)
	****	revise: menambahkan BC41 (on line: 226)
	*/
	...
	<head>
		...
		<script type="text/javascript">
			...
			Ext.onReady(function() {
				...
				var grid_opp = Ext.create('Ext.grid.Panel', {
					...
					tbar: [{
						...
						<?php if ($sesmis_level == 'ADMIN') { ?> {
								...
								id: 'btn_uploadbc25',
								iconCls: 'upload',
								text: 'Upload NoReg BC25',
								tooltip: 'Upload NoReg BC25',
								handler: fnc_uploadbc25
							}, '->',
							{
								xtype: 'button',
								id: 'btn_upload_general',
								iconCls: 'upload',
								text: 'Upload General BC',
								tooltip: 'Upload General BC',
								handler: fnc_upload
							},
						<?php	} ?> '->',
						{
				...
				//	form upload
				function fnc_uploadbc25() {
					var win_uploadbc25;
					if (!win_uploadbc25) {
						var frm_uploadbc25 = Ext.widget('form', {
							layout: {
								type: 'vbox',
								align: 'stretch'
							},
							...
							items: [{
								...
								value: '<?= $sesmis_userid ?>' + Ext.Date.format(new Date(), 'Ymd')
							}, {
							...
							buttons: [{
								...
								tooltip: 'Format CSV',
								handler: function() {
									window.open('response/form_noregbc25.php');
								}
							}, {
								text: 'Upload',
								iconCls: 'upload',
								...
								handler: function() {
									...
									Ext.Msg.confirm('Upload Data', 'ID From Data: <b>' + files + '</b> <br> Apakah Anda yakin ingin melakukan proses ini ?', function(btn) {
										if (btn == 'yes') {
											if (form.isValid()) {
												form.submit({
													url: 'response/upl_noregbc25.php',
						...
						win_uploadbc25 = Ext.widget('window', {
							title: 'Upload Update No Register',
							width: 450,
							minWidth: 450,
							height: 170,
							minHeight: 170,
							...
							constrain: true,
							layout: 'fit',
							animateTarget: 'btn_uploadbc25',
							items: frm_uploadbc25
						});
					}
					win_uploadbc25.show();
				}
				...
				function fnc_upload() {
					var win_upload_general;
					if (!win_upload_general) {
						var frm_upload_general = Ext.widget('form', {
							layout: {
								type: 'vbox',
								align: 'stretch'
							},
							...
							buttons: [{
								...
								tooltip: 'Format CSV',
								handler: function() {
									window.open('response/form_other_bc.php');
								}
							}, {
								text: 'Upload',
								iconCls: 'upload',
								...
								handler: function() {
									...
									Ext.Msg.confirm('Upload Data', 'ID From Data: <b>' + files + '</b> <br> Apakah Anda yakin ingin melakukan proses ini ?', function(btn) {
										if (btn == 'yes') {
											if (form.isValid()) {
												form.submit({
													url: 'response/upl_general_BC.php',
													waitMsg: 'sending data',
													submitEmptyText: false,
													success: function(form, action) {
														popwindow.close();
						...
						win_upload_general = Ext.widget('window', {
							title: 'Upload Data MANUAL',
							width: 450,
							minWidth: 450,
							height: 170,
							minHeight: 170,
							...
							constrain: true,
							layout: 'fit',
							animateTarget: 'btn_upload_general',
							items: frm_upload_general
						});
					}
					win_upload_general.show();
				}
				//	end of form upload
				//	----***----  //
			});
	...
	<body id="body">
		...
		<section>
			<div class="wrapper_section">
				...
				<div style="float:left;">OUTGOING PARTS PERMONTH</div>
				<div style="float:right"> Welcome: <?= $img . ' <b>' . $sesmis_username ?> </b></div>
				<br><br>
